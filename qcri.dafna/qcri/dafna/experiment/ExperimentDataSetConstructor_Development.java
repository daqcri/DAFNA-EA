...
public class ExperimentDataSetConstructor_Development {
	 ...
	 */
	public static DataSet readDataSet(double startingConfidence, double statingTrustworthiness, String dataSetDirectory, 
			double toleranceFactor, boolean cleanObjectId, ValueType objectIdValueType, 
			Experiment experiment, String syntheticDirectory, boolean MLE) {
		 ...
		 */
		DataSet dataSet = reader.readDirectoryFiles(dataSetDirectory, cleanObjectId, objectIdValueType, timings, Globals.delimiterText.charAt(0));
		/**
...
//			trueValueCount = truthReader.readDirectoryFiles();
			...
			trueValueCount = truthReader.readDirectoryFiles(Globals.directory_formattedDAFNADataset_PopulationTruthFolder, Globals.delimiterText.charAt(0));
...
//			trueValueCount = truthReader.readDirectoryFiles(); 
			CSVTruthReader truthReader = new CSVTruthReader(dqm.getDataItemMeasures());
			trueValueCount = truthReader.readDirectoryFiles(Globals.directory_formattedDAFNADataset_FlightTruthFolder, Globals.delimiterText.charAt(0));
		} else if (experiment == Experiment.Books) {
//			BooksTruthReader truthReader = new BooksTruthReader(dqm.getDataItemMeasures(), MLE);
//			trueValueCount = truthReader.readDirectoryFiles();
			CSVTruthReader truthReader = new CSVTruthReader(dqm.getDataItemMeasures());
			trueValueCount = truthReader.readDirectoryFiles(Globals.directory_formattedDAFNADataset_BooksTruthFolder, Globals.delimiterText.charAt(0));
...
//			trueValueCount = truthReader.readDirectoryFiles(Globals.directory_formattedDAFNADataset_WeatherTruthFolder);
			CSVTruthReader truthReader = new CSVTruthReader(dqm.getDataItemMeasures());
			trueValueCount = truthReader.readDirectoryFiles(Globals.directory_formattedDAFNADataset_WeatherTruthFolder, Globals.delimiterText.charAt(0));
